...
namespace future
{
    ...
    public class MainFrame : UIXmlEx, FCListenerMessageCallBack, FCMenuItemTouchEventCallBack, FCInvokeEventCallBack, FCTouchEventCallBack {
        ...
        /// 历史数据缓存
        /// </summary>
        private List<SecurityData2> m_securityDatasCache = new List<SecurityData2>();
        ...
        private List<FCScript> m_indicators = new List<FCScript>();

        /// <summary>
        /// 获取所有指标
        /// </summary>
        public List<FCScript> getIndicators() {
            return m_indicators;
        }
        ...
        /// </summary>
        /// <param name="value"></param>
        public void setIndicators(List<FCScript> value) {
            m_indicators = value;
        }
        ...
        public void bindHistoryData(String code, int cycle, List<SecurityData2> historyDatas) {
        ...
        private void newData(SecurityLatestData latestData)
        {
            lock (m_securityDatasCache)
            {
                ...
                FutureService.mergeLatestData(m_securityDatasCache, latestData, m_clientTickDataCache, getCycle());
                ...
                {
                    SecurityData2 securityData = m_securityDatasCache[i];
                    int[] fields = new int[] { KeyFields.CLOSE_INDEX, KeyFields.HIGH_INDEX, KeyFields.LOW_INDEX, KeyFields.OPEN_INDEX, KeyFields.VOL_INDEX, KeyFields.AMOUNT_INDEX, KeyFields.AVGPRICE_INDEX };
                ...
                if (message != null) {
                    if (message.m_serviceID == DataCenter.m_latestServiceClient.getServiceID()) {
                        ...
                        if (m_requestID2 == message.m_requestID) {
                            ...
                            List<SecurityData2> datas = HistoryServiceClient.getDatas(ref code, ref cycle, ref startDate, ref endDate, ref nowDate, ref nowVolume, ref nowAmount, message.m_body, message.m_bodyLength);
        ...
        public override void loadData() {
            ...
            Security security = new Security();
            security.m_code = "cu2303.SHFE";
            security.m_name = "cu2303";
            searchSecurity(security);
        }
        ...
        public override void load(String xmlPath) {
            loadFile(DataCenter.getAppPath() + "\\config\\MainFrame.xml", null);
            m_native = getNative();
            initInterface();
